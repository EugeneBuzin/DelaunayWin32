...
Инициализация начинается с чтения данных из исходного файла .node (см. функцию ReadNodes, которая начинается в строке 57 файла DelaunayInitialization.cpp). Проверяется существование входного файла

infile = _wfopen(nodefilename.c_str(), L"r");

см. строку 82 в файле DelaunayInitialization.cpp. (Здесь nodefilename это буфер для имени входного файла.) Если файл открывается, то выполняется чтение его содержимого. Иначе, выводится сообщение об ошибке и, и после того, как пользователь зарывает диалог с этим сообщением, осуществляется выход из программы.
