...
Предлогаемая здесь вашему вниманию программа (точнее её проект и исходный текст) имеет наименование Delaunayin32. Вкрадце (не описывая прелюдию создания), я создал её следующим образом. Взял программу "triangle.c" Джонотана Ричарда Шевчука https://github.com/libigl/triangle/blob/master/triangle.c. Взял из неё ту её часть, которая выполняет триангуляцию Деолоне по алгоритму "Разделяй и властвуй", перевёл взятый код с Си на C++, добавив в некоторых местах параллелизм (использование библиотеки PPL), "обернул" то что получилось в оконный интерфейс Win32 API и, в итоге, осмелился считать получившееся приложение (далее по тексту этого документа - программа) своей программой.
