...
Переменная 
static cancellation_token_source cancelTriSource;
объявлена в области видимости файла DelaunayWin32.cpp в строке 57. Это сделано из-за того, что доступ к ней, внутри оконной процедуры WndProc, осуществляется ещё из обработчика сообщения IDM_STOP - останова и отмены выполнения триангуляции, а объявлять cancelTriSource как локальную переменную в WndProc мне не хотелось, что бы "не засорять" оконную процедуру локальными переменными.
