...
Ниже приводится исходного текста входного файла на примере файла spiral.node.
...
Символом # помечаются коментарии. Первая строка содержит следующие данные (слева направо): написать. Каждая последующая строка содержит: порядковый номер вершины, координата X вершины, координата Y вершины.
...
- функция DelaunayWriteToFile::WriteElements, определённая в строках 126 - 233 файла DelaunayWriteToFile.cpp, записывает полученные при триангуляции треугольники в выходной файл *.ele, (данные по каждому из треугольников представлены в виде отдельной строки, содержащий порядковый номера треугольника и номера трёх рёбер, из которых состоит этот треугольник (эти номера представляют собой порядковые номера рёбер из выходного файла .edge);
- функция DelaunayWriteToFile::WriteEdges, определённая в строках 236 - 367 файла DelaunayWriteToFile.cpp, записывает полученные при триангуляции рёбра треугольников в выходной файл *.edge (данные по каждому из рёбер представлены в виде отдельной строки, содержащий порядковый номера ребр, номер вершины, с которой начинается ребро и номер вершины, которой заканчивается ребро (эти номера представляют собой порядковые номера вершин из выходного файла .node));
- функция DelaunayWriteToFile::WriteNeighbors, определённая в строках 236 - 367 файла DelaunayWriteToFile.cpp;
